...
bool WebServlet(const sofa::pbrpc::HTTPRequest& request,
  ...
  for (it = query_params.begin(); it != query_params.end(); ++it) {
    SLOG(INFO, "QueryParam[\"%s\"]=\"%s\"", it->first.c_str(),
         it->second.c_str());
  }
 ...
 private:
  virtual void Echo(google::protobuf::RpcController* controller,
    ...
    if (cntl->IsHttp()) {
      ...
      for (it = headers.begin(); it != headers.end(); ++it) {
        SLOG(INFO, "Header[\"%s\"]=\"%s\"", it->first.c_str(),
             it->second.c_str());
      }
    }
...
int main() {
  ...
  options.work_thread_init_func =
      sofa::pbrpc::NewPermanentExtClosure(&thread_init_func);
  options.work_thread_dest_func =
      sofa::pbrpc::NewPermanentExtClosure(&thread_dest_func);
  sofa::pbrpc::RpcServer rpc_server(options);

  sofa::pbrpc::Servlet servlet =
      sofa::pbrpc::NewPermanentExtClosure(&WebServlet);
  ...
  // Delete closures.
  // Attention: should delete the closures after server stopped, or may be
  // crash.
